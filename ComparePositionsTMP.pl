...
#	print STDERR "$cell $t XYZ $xnew $ynew $z XRtheta $xnew $r{$cell}{$t} $theta\n";
    ...
    open (IN, "$dir/$fname" . "_positions.txt") || die "Couldn't read $fname\n";
    open (DEV, ">$dir/$fname" . "_deviations.txt") || die "Couldn't write $fname deviations\n";
    open (DIST, ">$dir/$fname" . "_distance.txt") || die "Couldn't write $fname distance\n";
    foreach(<IN>){
