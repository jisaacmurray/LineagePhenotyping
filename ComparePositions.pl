...
#    print STDERR "PROCESSING $series\n";
    ...
    my $SCDfile = $DBref->{$series}{"annots"} . "/dats/ACD$series.csv";
...
#    print STDERR "$series XYRES $xyres ZRES $zres\n";
    ...
    #for ACD file version - already in microns
    $xyres=1;
    $zres=1;
...
sub OutputPositions{
    ...
    open(OUT, ">$dir/$fname" . "positions.txt") || die "Couldn't open file $dir/$fname for output\n";
    open(RADIAL, ">$dir/$fname" . "radialpositions.txt") || die "Couldn't open file $dir/$fname for radial output\n";
...
#	print STDERR "$cell $t XYZ $xnew $ynew $z XRtheta $xnew $r{$cell}{$t} $theta\n";
    ...
    close RADIAL;
    return;
    open (IN, $fname . "_positions.txt") || die "Couldn't read $fname\n";
    open (DEV, ">$fname" . "_deviations.txt") || die "Couldn't write $fname deviations\n";
    open (DIST, ">$fname" . "_distance.txt") || die "Couldn't write $fname distance\n";
    foreach(<IN>){
