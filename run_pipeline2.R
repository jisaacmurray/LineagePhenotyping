...
wt_ref_dir  <- if (!is.null(cfg$wt_ref_dir)) .resolve(cfg$wt_ref_dir, script_dir) else file.path(script_dir, "Richard_et_al_plus_comma_WT")
...
# Update RUNNER_DIR to use script_dir
...
# CWD is set to script_dir so bare source() calls in helper scripts
# resolve correctly against the script directory.
# ------------------------------------------------------------------
setwd(RUNNER_DIR)
...
message(sprintf("[run_pipeline] script_dir = %s", script_dir))
