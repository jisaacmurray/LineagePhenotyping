...
#UNCOMMENT THIS AND THE MAKEDB REFS ABOVE TO GRAB FRESH CD FILES
    ...
    my $CDFile="CD" . $_ . ".csv";
    my $CDOutFile=$db->{$_}{"annots"} . "/dats/ACD" . $_ . ".csv";
...
#		print "$cell $observed $standard, " . $standard/$observed . "\n";
	...
	#derotate (CONSIDER DESTRETCHNG AS WELL)
	...
	$x=$x/($maj/$stdMaj);
	$y=$y/($min/$stdMin);
	#FIXME figure out z destretching
...
#	$z=$z+33.5;
	...
	#flip axis to match ADL standard (currently uses centered at zero)
	if($axis eq "AVR"){
	    $z=-$z;
	    $y=-$y;
	}elsif($axis eq "PVL"){
...
#	print OUT join("\t", $CT, $cell, $time, $e1, $e2, $e3, $e4, $e5, $z, $x, $y, $d, $gweight) . "\n";
	...
	if($division{$_}=~/\d/){#Cell supposed to divide
		...
		if($altLength<$length){ 
		    ...
		    print STDERR "$_ should have divided but didn't!\n";
		}
	    }else{
